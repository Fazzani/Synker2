...
namespace SyncLibrary.TvgMediaHandlers
{
    ...
    {
        public TvgMediaEpgMatcherNameHandler(IContextTvgMediaHandler contextTvgMediaHandler) : base(contextTvgMediaHandler)
        {
        ...
        {
            var result = ElasticConnectionClient.Client.SearchAsync<tvChannel>(x => x.From(0).Size(1).Query(q => q.Match(m => m.Field(f => f.displayname).Query(tvgMedia.Name)))).GetAwaiter().GetResult();
