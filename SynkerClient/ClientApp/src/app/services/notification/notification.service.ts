...
export class NotificationService extends BaseService {
  ...
  public list(userId: string, limit: number = 10): AngularFireList<FirebaseNotification> {
    return this.db.list<FirebaseNotification>(`/notifications/${userId}`, ref => ref.limitToFirst(limit).orderByChild('Order'));
  }

  public count(userId: string): Observable<number> {
    return this.db
  ...
  public remove(userId: string, key: string) {
