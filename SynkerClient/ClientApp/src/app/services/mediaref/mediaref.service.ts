...
export class MediaRefService extends BaseService {
    ...
    list(query: ElasticQuery): Observable<ElasticResponse<mediaRef>> {
      return this.http.post(environment.base_api_url + 'mediasref/_search', query).map(this.handleSuccess).catch(this.handleError);
    }

    synk(): Observable<ElasticResponse<mediaRef>> {
      return this.http.post(environment.base_api_url + 'mediasref/synk', null).map(this.handleSuccess).catch(this.handleError);
    }

    synkPicons(): Observable<ElasticResponse<mediaRef>> {
      return this.http.post(environment.base_api_url + 'mediasref/synkpicons', null).map(this.handleSuccess).catch(this.handleError);
    }

    save(...medias: mediaRef[]): Observable<ElasticResponse<mediaRef>> {
      return this.http.post(environment.base_api_url + this.BaseUrl, medias).map(this.handleSuccess).catch(this.handleError);
    }
    ...
    groups(filter: string): Observable<ElasticAggregations> {
      return this.http.post(environment.base_api_url + 'mediasref/groups', filter).map(this.handleSuccess).catch(this.handleError);
    }
    ...
    */
    cultures(filter?: string): Observable<string[]> {
      ...
      return this.http.get(environment.base_api_url + 'mediasref/cultures?filter=' + f).map(this.handleSuccess).catch(this.handleError);
    }
   ...
   */
    tvgSites(): Observable<sitePackChannel[]> {
        return this.http.get<sitePackChannel[]>(environment.base_api_url + 'sitepack/tvgsites').map(res => {
            return res;
 ...
 */
    sitePacks(filter: string): Observable<sitePackChannel[]> {
        return this.http.get<sitePackChannel[]>(environment.base_api_url + 'mediasref/sitepacks?filter=' + filter).map(res => {
            return res;
        }, err => this.handleError(err));
    }
}
