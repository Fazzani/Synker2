...
export class PlaylistService extends BaseService {
    ...
    list(query: QueryListBaseModel): Observable<PagedResult<PlaylistModel>> {
      return this.http.post(`${variables.BASE_API_URL}${this.BaseUrl}/search`, query).map(this.handleSuccess).catch(this.handleError);
    }

    synk(model: PlaylistPostModel): Observable<PlaylistModel> {
      return this.http.post(`${variables.BASE_API_URL}${this.BaseUrl}/synk`, model).map(this.handleSuccess).catch(this.handleError);
    }

    executeHandlers(model: TvgMedia[]): Observable<TvgMedia[]> {
      return this.http.post(`${variables.BASE_API_URL}${this.BaseUrl}/handlers`, model).map(this.handleSuccess).catch(this.handleError);
    }

    match(id: string): Observable<PlaylistModel> {
      return this.http.post(`${variables.BASE_API_URL}${this.BaseUrl}/match/$${id}`, null).map(this.handleSuccess).catch(this.handleError);
    }

    matchtvg(id: string, onlyNotMatched: boolean = true): Observable<PlaylistModel> {
      return this.http.post(`${variables.BASE_API_URL}${this.BaseUrl}/matchtvg/${id}?onlyNotMatched=${onlyNotMatched}`, null).map(this.handleSuccess)
        .catch(this.handleError);
    }

    matchFiltredTvgSites(id: string, onlyNotMatched: boolean = true): Observable<PlaylistModel> {
      return this.http.post(`${variables.BASE_API_URL}${this.BaseUrl}/matchfiltred/${id}?onlyNotMatched=${onlyNotMatched}`, null).map(this.handleSuccess)
        .catch(this.handleError);
    }
    ...
    */
    matchVideosByPlaylist(id: string): Observable<PlaylistModel> {
      return this.http.post(`${variables.BASE_API_URL}${this.BaseUrl}/matchvideos/${id}`, null).map(this.handleSuccess).catch(this.handleError);
    }
    ...
    matchVideos(...medias: TvgMedia[]): Observable<TvgMedia[]> {
      return this.http.post(`${variables.BASE_API_URL}${this.BaseUrl}/matchvideos`, medias).map(this.handleSuccess).catch(this.handleError);
    }
    ...
    matchVideo(mediaName: string): Observable<any> {
      return this.http.post(`${variables.BASE_API_URL}${this.BaseUrl}/matchvideo/${mediaName}`, null).map(this.handleSuccess).catch(this.handleError);
    }
    ...
    diff(model: PlaylistPostModel): Observable<PlaylistModel> {
      return this.http.post(`${variables.BASE_API_URL}${this.BaseUrl}/diff`, model).map(this.handleSuccess).catch(this.handleError);
    }
