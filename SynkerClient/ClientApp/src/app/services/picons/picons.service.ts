...
export class PiconService extends BaseService {
  ...
  get(id: string): Observable<ElasticResponse<picon>> {
    return this.http.get(`${environment.base_api_url}${this.BaseUrl}/${id}`)
    ...
    list(field: string, filter: string): Observable<ElasticResponse<picon>> {
      ...
      return this.http.post(`${environment.base_api_url}${this.BaseUrl}/_search`, q).map(this.handleSuccess).catch(this.handleError);
    }

    delete(id: string): Observable<number> {
      return this.http.delete(`${environment.base_api_url}${this.BaseUrl}/${id}`).map(this.handleSuccess).catch(this.handleError);
    }

    synk(): Observable<ElasticResponse<picon>> {
      return this.http.post(`${environment.base_api_url}${this.BaseUrl}/synk`, null).map(this.handleSuccess).catch(this.handleError);
    }

    match(model: TvgMedia[], distance: number = 90, shouldMatchChannelNumber: boolean = true): Observable<TvgMedia[]> {
      return this.http.post(`${environment.base_api_url}${this.BaseUrl}/match?distance=${distance}&shouldMatchChannelNumber=${shouldMatchChannelNumber}`, model)
        .map(this.handleSuccess)
        .catch(this.handleError);
    }
}
