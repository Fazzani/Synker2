namespace PlaylistManager.Entities
{
    ...
    public class Media : IComparable<Media>, IComparable, IValidatableObject, IEquatable<Media>, IEqualityComparer<Media>
    {
        ...
        public string Id
        {
            get
            {
                if (_iid == null && !string.IsNullOrEmpty(Url))
                    _iid = Convert.ToBase64String(Encoding.UTF8.GetBytes($"{new Uri(Url).IdnHost}{new Uri(Url).PathAndQuery}{Name}"), Base64FormattingOptions.None);
                return _iid;
            }
            set
            {
